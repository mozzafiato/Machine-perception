...
%IM2 = IM2(1:min(size(IM2,1), size(IM3,1)), :, :);
%IM3 = IM3(1:min(size(IM2,1), size(IM3,1)), :, :);

M = find_matches(rgb2gray(IM3), rgb2gray(IM2),sigma1, sigma2, t, bins, m);
M = M(1:5,:);
%figure; displaymatches(IM3, M(:,1), M(:,2), IM2, M(:,3), M(:,4));
H = estimate_homography(M(:,1), M(:,2), M(:,3), M(:,4));
...
M = M(1:5,:);
H = estimate_homography(M(:,1), M(:,2), M(:,3), M(:,4));
figure; displaymatches(IM1, M(:,1), M(:,2), IG1, M(:,3), M(:,4));
IG2 = merge_homography(IM1,IG1, H);
figure; imshow(IG2(1:end-50,1:end-200,:));
